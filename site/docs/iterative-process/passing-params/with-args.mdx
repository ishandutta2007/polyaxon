...
You should notice that the CLI uses the following format: `kind:value` to pass [hyperparameters](/docs/references/polyaxonfile/automation/matrix/params/), in this case it passes the `choice` kind.
Another important aspect to notice is that matrix is of kind [grid search](/docs/references/polyaxonfile/automation/matrix/grid-search/) and it runs the operations sequentially by setting the `concurrency` to `1`.
